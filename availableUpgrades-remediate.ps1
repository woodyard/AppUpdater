...
function Remove-StaleScheduledTasks {
    ...
    # Active prefixes the script creates now (v9.64):
    #   - UserRemediation_  : Schedule-UserContextRemediation
    #   - DialogHost_       : Start-DialogHost
    # Legacy per-prompt prefixes kept here so the sweep still removes any
    # leftover tasks from pre-v9.64 versions still present on machines that
    # upgraded - they will not be created anew but may exist on disk.
    $prefixes = @(
        "UserRemediation_",
        "DialogHost_",
        "UserPrompt_",
...
$ScriptTag = "68" # Keep in sync with the Tag field in the header .NOTES block
...
if (-not $whitelistUrl) {
    # v9.68: was woodyard/public-scripts/main/remediations - stale fork of the
    # whitelist where some apps (e.g. Anthropic.Claude) are still enabled, while
    # AppUpdater has them Disabled. Detect and remediate must point at the same
    # whitelist or remediate silently drops entries detect put in the task file.
